...
class FoVAgent(BaseAgent):
    ...
    async def edit_node(self, state: ArtistState, config=None):
        """进入绘制阶段。"""
        logger.info("[edit] 进入")
        user_prompt = self._user_prompt["edit_node"].format(edit_draft=state.history_drafts[-1])
    
        ## load multi-modal messages 
        
        logger.info(f"[edit_node] 📝 user_prompt: {user_prompt}")
        ## from bytes to PIL Image
        raw_ref_image = bytes_to_raw_reference_image(state.history_images[0])
        ...
        return {
                "history_images": [open(os.path.join(state.project_dir, "edit_node.png"), "rb").read()],
        }
